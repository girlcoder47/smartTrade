...
balFile = 'all/'+mkt+'/trade/'+ticker+'_'+ind+'_balance.csv'
txnFile = 'all/'+mkt+'/trade/'+ticker+'_'+ind+'_txn.csv'

startBal=10000

if not os.path.isfile(balFile):
	cash=10000
	qty=0
	purPrice=0

	balRows={'date':execDate,'Cash':cash,'Qty':0,'PurchasePrice':0,'CurrentPrice':0,'CurrentEquityTotal':0,
				 'PortfolioTotal':startBal,'PercentageChange':0}

	tmpBalList.append(balRows)
	dfBal =  pd.DataFrame.from_dict(tmpBalList)
	dfBal.to_csv(balFile,mode='w',header=True, index=False)
else:
